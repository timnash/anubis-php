...
function anubis_keySetup($key) {
    global $T0, $T1, $T2, $T3, $T4, $T5, $rc;
	$N = strlen($key)*4; //convert hex length to binary length 0->0000 -> F->1111
    ...
    $keyStruct = new stdClass();
    $keyStruct->R = $R;

    $key = str_split($key,2);//This handles the hex string by breaking it up into bytes
	...
	for($r = 0; $r <= $R; $r++) {
        ...
        $keyStruct->roundKeyEnc[$r][0] = $K0;
        $keyStruct->roundKeyEnc[$r][1] = $K1;
        $keyStruct->roundKeyEnc[$r][2] = $K2;
        $keyStruct->roundKeyEnc[$r][3] = $K3;
    ...
    for ($i = 0; $i < 4; $i++) {
        $keyStruct->roundKeyDec[0][$i] = $keyStruct->roundKeyEnc[$R][$i];
        $keyStruct->roundKeyDec[$R][$i] = $keyStruct->roundKeyEnc[0][$i];
    }
    for ($r = 1; $r < $R; $r++) {
        for ($i = 0; $i < 4; $i++) {
            $v = $keyStruct->roundKeyEnc[$R - $r][$i];
            $keyStruct->roundKeyDec[$r][$i] = 
                $T0[$T4[($v >> 24)       ] & 0xff] ^
...
function anubis_crypt($plaintext, $roundKey, $R) {
    global $T0, $T1, $T2, $T3, $T4, $T5, $rc;
    $plaintext = str_split($plaintext,2);//split plaintext hex string into bytes

    for ($i = 0, $pos = 0; $i < 4; $i++, $pos += 4) {
        $state[$i] = 
            (hexdec($plaintext[$pos    ]) << 24) ^
            (hexdec($plaintext[$pos + 1]) << 16) ^
            (hexdec($plaintext[$pos + 2]) <<  8) ^
            (hexdec($plaintext[$pos + 3])      ) ^
            $roundKey[0][$i];
    }
    ...
    for ($i = 0, $pos = 0; $i < 4; $i++, $pos +=4) {
        $w = $inter[$i];
        $ciphertext[$pos    ] = dechex(ord(pack('C', $w >> 24)));
        $ciphertext[$pos + 1] = dechex(ord(pack('C', $w >> 16)));
        $ciphertext[$pos + 2] = dechex(ord(pack('C', $w >>  8)));
        $ciphertext[$pos + 3] = dechex(ord(pack('C', $w      )));
    }
...
function anubis_encrypt($plaintext) {
    $random = bin2hex(secure_random_bytes(64));
    $keyStretchSalt = bin2hex(secure_random_bytes(64));
    $keyStretched = scrypt($random, $keyStretchSalt, 65536, 8, 1, 104);
    $key = substr($keyStretched, 0, 80);
    $keyHMAC = substr($keyStretched, 80);
    $iv = bin2hex(secure_random_bytes(16));
    $keyStruct = anubis_keySetup($key);
    $hex_plaintext = bin2hex($plaintext);
    $hexplain_array = str_split($hex_plaintext,32);
    if(strlen($hexplain_array[count($hexplain_array)-1])!==32){
        $hexplain_array[count($hexplain_array)-1] .= "8";
        while(strlen($hexplain_array[count($hexplain_array)-1])<32){
            $hexplain_array[count($hexplain_array)-1] .= "0";
        }
    }
    $ciphertext = "";
    $hexplain_array[0] = large_xor($hexplain_array[0], $iv);
    for ($i=0; $i < count($hexplain_array); $i++) { 
            $current_ciphertext = anubis_crypt($hexplain_array[$i], $keyStruct->roundKeyEnc, $keyStruct->R);
            if(array_key_exists($i+1, $hexplain_array)){
                $hexplain_array[$i+1] = large_xor($hexplain_array[$i+1],$current_ciphertext);
            }
    ...
    $okeypad = large_xor($opad,$keyHMAC);
    $ikeypad = large_xor($ipad,$keyHMAC);
    $hmac = scrypt($okeypad . scrypt($ikeypad . $ciphertext, $key, 65536, 8, 1, 256), $key, 65536, 8, 1, 256);
    $returnArray = array(
        'key' => $key,
        'keyHMAC' => $keyHMAC,
        'iv' => $iv,
...
function anubis_decrypt($key, $iv, $keyHMAC, $hmac, $ciphertext) {
    $opad = "5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c";
    $ipad = "36363636363636363636363636363636363636363636363636363636363636363636363636363636363636363636363636363636363636363636363636363636";
    $okeypad = large_xor($opad,$keyHMAC);
    $ikeypad = large_xor($ipad,$keyHMAC);
    $hmacTEST = scrypt($okeypad . scrypt($ikeypad . $ciphertext, $key, 65536, 8, 1, 256), $key, 65536, 8, 1, 256);
    if($hmacTEST===$hmac){
        $keyStruct = anubis_keySetup($key);
        $ciphertext_array = str_split($ciphertext,32);
        array_unshift($ciphertext_array,$iv);
        $plaintext = "";
        for ($i=1; $i < count($ciphertext_array); $i++) {      
            $current_plaintext = anubis_crypt($ciphertext_array[$i], $keyStruct->roundKeyDec, $keyStruct->R);
            $plaintext .= large_xor($current_plaintext, $ciphertext_array[$i-1]);
        }
        $plaintext = rtrim($plaintext,"80");
        return hex2bin($plaintext);
    } else {
        return "Ciphertext has failed Authentication.";
    }

}


function large_xor($first, $second) {
    $first_array = str_split($first, 8);
    $second_array = str_split($second, 8);
    $final_array = array();
    foreach ($first_array as $i => $f) {
        $final_array[$i] = dechex(hexdec($f) ^ hexdec($second_array[$i]));
    }
